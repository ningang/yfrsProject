...
var validate = {
    //{name:}
    rulesDefault:function() {
        return {
            name:{required:true,max:50},
            sort:{number:true,min:1,max:255},
        }
    },
    messagesDefault:function() {
        return {
            name:{required:'请输入名称',max:'最大长度不能超过50'},
            name:{number:'请输入数字',min:"数值最小为1",max:'数值最大不能超过255'},
        }
    }
    validate:function(dom,rules,messages){
        if(typeof dom == 'string') throw new Error(dom +'is string,please use form element');
        if(!rules) rules = this.rulesDefault();
        if(!message) messages = this.messagesDefault();
    }
};
yfrs.extend()